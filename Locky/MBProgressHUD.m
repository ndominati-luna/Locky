...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.mainTintColor = [UIColor blackColor];
...
//		self.detailsLabelFont = [UIFont systemFontOfSize:kDetailsLabelFontSize];
		self.detailsLabelFont = [UIFont preferredFontForTextStyle:UIFontTextStyleCaption2];
...
-(void)  hide:(BOOL)animated completion:(void (^) (void)) completion
{
    if (animated) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            [NSThread sleepForTimeInterval:0.3]; //time of the animation
...
//    label.layer.shadowOpacity = 0.75;
	label.textColor = self.mainTintColor;
	...
	detailsLabel.textColor = self.mainTintColor;
...
// Customization
			[(MBRoundProgressView *)self.indicator setBackgroundTintColor:[UIColor clearColor]];
			[(MBRoundProgressView *)self.indicator setProgressTintColor:self.mainTintColor];
...
- (void) blurBackground {
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(CGRectGetWidth(self.parent.frame), CGRectGetHeight(self.parent.frame)), NO, [[UIScreen mainScreen] scale] );
...
// Customizaton.
    //components.tintColor = [UIColor colorWithWhite:0.8f alpha:.9f];
	components.tintColor =  [[UIColor blackColor] colorWithAlphaComponent:0.5f];
...
@implementation UIImage (HUDImageEffects)
...
//    outputImage = [UIImage imageWithCGImage:imageRef scale:[UIScreen mainScreen].scale orientation:UIImageOrientationUp];
...
//    outputImage = [UIImage imageWithCGImage:destRef scale:[UIScreen mainScreen].scale orientation:UIImageOrientationUp];
