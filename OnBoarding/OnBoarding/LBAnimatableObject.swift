...
extension  UIView  {
    ...
    class func setAnimationProgressOnObject( animatableObject : LBAnimatableObject , progress : CGFloat , animated : Bool , delay : Float) {
        if   animated {
            
            UIView.animateWithDuration(NSTimeInterval(animatableObject.aDuration), delay: NSTimeInterval(animatableObject.delay) + NSTimeInterval(delay) , usingSpringWithDamping: animatableObject.dumping, initialSpringVelocity: animatableObject.initialVelocity, options: UIViewAnimationOptions.allZeros, animations: { () -> Void in
